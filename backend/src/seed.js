...
function seedFeedingRecords() {
  ...
  const insertRecord = db.prepare(`
    INSERT INTO feeding_records (feeding_date, location, cat_food_type, quantity, weather, remark)
    VALUES (?, ?, ?, ?, ?, ?)
  `);
  ...
  try {
    for (const r of records) {
      insertRecord.run(r.feeding_date, r.location, r.cat_food_type, r.quantity, r.weather, r.remark);
    }
    db.exec('COMMIT');
  } catch (err) {
    db.exec('ROLLBACK');
...
function seedCatFeedingRecords() {
  ...
  const insertRecord = db.prepare(`
    INSERT INTO cat_feeding_records (cat_sighting_id, feeding_date, food_type, quantity, remark)
    VALUES (?, ?, ?, ?, ?)
  `);
  ...
  try {
    for (const r of records) {
      insertRecord.run(r.cat_sighting_id, r.feeding_date, r.food_type, r.quantity, r.remark);
    }
    db.exec('COMMIT');
  } catch (err) {
    db.exec('ROLLBACK');
